...
do -- Turret drives
	...
	local WillUseSmallModel = function(Size)
		return Size <= 12.5
	end
	...
	Turrets.Register("1-Turret", {
		...
		CalcSpeed = function(TurretData, PowerData) -- Called whenever something on the turret changes, returns resistance from mass on the ring (overall, not inertial)
			...
			if TurretData.TotalMass > TurretData.MaxMass then
				OverweightMod =
					math.max(0, 1 - (((TurretData.TotalMass - TurretData.MaxMass) / TurretData.MaxMass) / 2))
			end
			...
			if (math.max(1, ReqConstantPower) / math.max(MaxPower, 1)) > 1 then
				return { SlewAccel = 0, MaxSlewRate = 0 }
			end -- Too heavy to rotate, so we'll just stop here
			...
			if (math.max(1, ReqAccelPower) / math.max(1, Accel)) > 5 then
				return { SlewAccel = 0, MaxSlewRate = 0 }
			end -- Too heavy to accelerate, so we'll just stop here

			local FinalAccel = Accel * math.Clamp(MaxPower / ReqAccelPower, 0, 1) * 6 -- converting back to deg/s^2

			return {
				SlewAccel = FinalAccel,
				MaxSlewRate = FinalTopSpeed,
				MotorMaxSpeed = TopSpeed * 6,
				MotorGearRatio = GearRatio,
				EffortScale = math.min(1, 1 / (MaxPower / ReqConstantPower)),
			}
		end,
	})
		...
		Turrets.RegisterItem("Turret-H", "1-Turret", {
			...
			SlewFuncs = {
				GetStab = function(Turret)
					if (not (Turret.Stabilized and Turret.Active)) or (Turret.Manual == true) then
						return 0
					end
				...
				GetTargetBearing = function(Turret, StabAmt)
					...
					if TurretTbl.HasArc then
						if TurretTbl.Manual then
							return Rotator:WorldToLocalAngles(
								Turret:LocalToWorldAngles(
									Angle(0, -math.Clamp(TurretTbl.DesiredDeg, TurretTbl.MinDeg, TurretTbl.MaxDeg), 0)
								)
							).yaw
						else
							local AngDiff = Rotator:WorldToLocalAngles(TurretTbl.LastRotatorAngle)
							local LocalDesiredAngle = ClampAngle(
								Turret:WorldToLocalAngles(TurretTbl.DesiredAngle) - Angle(0, StabAmt, 0) - AngDiff,
								Angle(0, -TurretTbl.MaxDeg, 0),
								Angle(0, -TurretTbl.MinDeg, 0)
							)
						...
						local AngDiff = Rotator:WorldToLocalAngles(TurretTbl.LastRotatorAngle)
						return TurretTbl.Manual
								and Rotator:WorldToLocalAngles(
									Turret:LocalToWorldAngles(Angle(0, -TurretTbl.DesiredDeg, 0))
								).yaw
							or (Rotator:WorldToLocalAngles(TurretTbl.DesiredAngle + AngDiff).yaw - StabAmt)
					end
				end,
				...
				GetTargetBearing = function(Turret, StabAmt)
					...
					if TurretTbl.HasArc then
						if TurretTbl.Manual then
							return Rotator:WorldToLocalAngles(
								Turret:LocalToWorldAngles(
									Angle(math.Clamp(-TurretTbl.DesiredDeg, TurretTbl.MinDeg, TurretTbl.MaxDeg), 0, 0)
								)
							).pitch
						else
							local LocalDesiredAngle = ClampAngle(
								Turret:WorldToLocalAngles(TurretTbl.DesiredAngle) - Angle(StabAmt, 0, 0),
								Angle(-TurretTbl.MaxDeg, 0, 0),
								Angle(-TurretTbl.MinDeg, 0, 0)
							)
						...
						return TurretTbl.Manual
								and Rotator:WorldToLocalAngles(
									Turret:LocalToWorldAngles(Angle(-TurretTbl.DesiredDeg, 0, 0))
								).pitch
							or (Rotator:WorldToLocalAngles(TurretTbl.DesiredAngle).pitch - StabAmt)
					end
				end,
